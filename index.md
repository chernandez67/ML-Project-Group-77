...
# Methods
We identified a dataset on Kaggle that collects artwork for the 50 most (subjectively) influencial artists, along with their respective, time period, style, and other classifiers. Based on this choice of dataset, many of these classifiers can already be used as clusters, such as 50 artist clusters. As a result, we can apply different unsupervised learning techniques to the full data set with a known number of clusters to see if different clustering algorithms are able to correctly group images by the classifer. Scikit Learn provides many implementations for these algorithms already that we can use to perform this analysis. Additionally, a very common approach to supervised learning with images is a Convolution Neural Network, which can be used to identify information about these artworks. Support vector machines are an especially powerful algorithm for classification, and may be better suited toward solving our problem. We will choose to use one of these models based on future research and/or timeline constraints. Going forward for both supervised and unsupervised approaches, it may be necessary to simplify the problem to a smaller group of artists or styles first (such as differentiating between only 2 of them) and then expand outward given the lessons learned from the simplified model.
